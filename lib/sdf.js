...
function ringsToSDF(rings, width, height, cutoff) {
    var tree = rbush(9);
    var offset = 0.5;
    var radius = 8;
...
exports.glyphToSDF = function(glyph, fontSize, buffer, cutoff) {
    ...
    info.data = ringsToSDF(rings, info.width, info.height, cutoff);

    return info;
};

exports.pathToSDF = function(path, width, height, cutoff) {
    return ringsToSDF(pathToRings(path), width, height, cutoff);
};
